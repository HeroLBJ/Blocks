...
//                        String rsRandom=getRsRandom();
//                        String rsKey="4d3ca962b11b934331584f2fb66b205d";
//                        builder.addHeader("rs-key",rsKey);
//                        builder.addHeader("rs-random",rsRandom);
//                        Request originalRequest = chain.request();
//                        HttpUrl url = originalRequest.url();
//                        String path = url.toString();
//                        String replace = path.replace(BASE_URL, "/");
//                        if (replace.contains("?")) {
//                            path = replace.substring(0, replace.lastIndexOf("?"));
//                        }else {
//                            path=replace;
//                        }
                        //MD5（MD5(“/api/values+111”.ToLower()).ToLower()+“+789”）
//                        String rsSign= AESUtils.md5(AESUtils.md5(path.toLowerCase()+"+"+rsKey)+"+"+rsRandom);
//                        builder.addHeader("rs-sign",rsSign);
//
//                        if (null!= TokenManager.getInstance().getToken()&&
//                                !TokenManager.getInstance().getToken().isEmpty()) {
//
//                            Log.e("AUTHORIZATION","AUTHORIZATION :"+
//                                    TokenManager.getInstance().getUserInfoModel().getToken().getToken_type()
...
//                                            +" "+TokenManager.getInstance().getToken());
//                        }
                        builder.addHeader("sourceport","android");
